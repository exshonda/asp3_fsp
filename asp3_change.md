...
（注）一時的に「TE 非トグル送信（TDR 直接書き込み＋TX_DATA_EMPTY 駆動）」も
試行したが，残存 1 バイト(0xFE)は TE トグルではなく再 Open グリッチ由来であること
が実機で判明し（送信方式を変えても 0xFE は消えない），送信経路は実績のある
FSP 標準の `R_SCI_B_UART_Write`＋`TX_COMPLETE` 駆動（従来方式）のままとした。
...
残存 1 バイトは上記 (A) の SCI_B 再 Open 時ハードウェア特性によるもので，
`target_serial.c` に閉じた対応の限界として確定（ユーザ合意済み）。

---

## Step 7: EK-RA8M2 出力停止（間欠スタール）修正 — HRT タイマドライバの3バグ

対象ファイル: `asp3/target/ek_ra8m2/target_timer.c` のみ（カーネル・FSP 本体は未変更）

### 症状

CPU は動作しているのにシリアル出力が起動後しばらくして停止する（数秒〜数十秒，
リセットによっては発生しない・継続時間も変わる間欠不具合）。Step 6 以前の
オリジナルコードから存在し，文字化け修正とは独立。その他の症状として，
task1 の進行速度がリセット毎に桁違いに変動（task_loop キャリブレーションの破壊），
"no time event is processed in hrt interrupt." 警告の洪水（ブート依存）も確認。

### 根本原因（3件，いずれも実機計測で確定）

GPT0 を 100MHz（GPTCLK = PLL2P 600MHz / 6，1 count = 10ns，32bit で約 42.9 秒周期）
のフリーランカウンタとして高分解能タイマ(HRT)に使用している。

1. **GTCNT の torn read（最重要）**: GPT のカウントクロックは専用 GPTCLK，レジスタ
   アクセスは別バスクロックドメイン。非同期境界をまたいで更新中の 32bit カウンタを
   読むと，まれに実カウントと大きく異なる不整合値が読める。PLL 位相がブート毎に固定
   されるため「ほぼ毎ティック化けるブート／ほぼ正常なブート」に二極化する。
   これにより `target_hrt_get_current` の返す時刻が前後にジャンプし，
   カーネルが「未来のイベント」を待ち続ける（＝出力停止）・task_loop 計測が壊れる・
   後述ガードが誤発火して警告洪水，が発生していた。
   （計測根拠: set_event 内の連続 2 回のカウンタ読みで「10ms 経過」を観測＝物理的に不可能）

2. **`target_hrt_set_event` の追い越しガード欠如**: コンペア値 (GTCNT+100·hrtcnt) を
   設定し終える前にカウンタが目標値を通過すると，一致は次の 32bit ラップまで発生せず
   HRT 割込みが最大 42.9 秒来ない。ASP3 の規約（設定時点で指定時刻が経過済みなら
   割込みを即時要求）に未対応だった。
   スタールが 42.9 秒未満の任意長で「自然回復」していたのは，GTCNT が古い GTCCRA 値に
   偶然再ヒットしてスプリアス割込みが入るため（停止長 = 旧コンペア値までの残距離）。

3. **`target_hrt_get_current` の非アトミック読み**: `upper_counter`（オーバーフロー
   割込みで更新）と GTCNT の読出しの間にオーバーフローが入ると約 42.9 秒ずれた時刻を
   返す。

### 修正内容

1. **GTCNT 一貫読みヘルパ `read_gtcnt_consistent()`**: 連続 2 回の読み値の差が
   許容範囲（200 counts = 2µs）に収まるまで読み直し，正常値のみ使用する。
   `target_hrt_get_current`・`target_hrt_set_event` の全カウンタ読みをこれに統一。

2. **set_event 追い越しガード**: コンペア設定後にカウンタを再読みし，設定中に目標値を
   通過していたら `target_hrt_raise_event()` で割込みを即時要求する。

3. **get_current のオーバーフロー整合**: `upper_counter` を volatile 化し，
   GTCNT 読出しの前後で `upper_counter` が不変であることを確認するまで読み直す
   （オーバーフロー割込みは NVIC 優先度 0＝CPU ロックでもマスクされないため，
   このループは長時間回らない）。

### 結果（実機 115200 8N1，最終ビルドで検証）

- リセット 5 回＋最終 3 回：全回で警告 0・task1 一定速度（2.5 回/秒）・無受信ギャップ
